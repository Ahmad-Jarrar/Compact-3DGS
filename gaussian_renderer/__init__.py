...
def render_ensemble_result(viewpoint_camera, pcs : list[GaussianModel], pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, itr=-1, rvq_iter=False):
    """
    Render the scene. 
    ...
    rendered_image = torch.stack(images, dim=0).mean(dim=0)
    return {"render": rendered_image,
            "viewspace_points": screenspace_points,
            "visibility_filter" : radii > 0,
            "radii": radii
...
# def render_ensemble_color(viewpoint_camera, pcs : list[GaussianModel], pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, itr=-1, rvq_iter=False):
    """
    Render the scene. 
